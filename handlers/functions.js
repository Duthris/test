...
function onCoolDown(message, command) {
   ...
   */
   ...
   async function swap_pages(client, message, description, TITLE) {
     ...
     const collector = swapmsg.createMessageComponentCollector({filter: (i) => i.isButton() && i.user && i.user.id == cmduser.id && i.message.author.id == client.user.id, time: 180e3 }); 
  ...
  function change_status(client) {
    try {
      client.user.setActivity('© | Maymunlar Dünyası', {
        type: "WATCHING",
      });
    } catch (e) {
      console.log(String(e.stack).bgRed)
